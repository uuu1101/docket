...
- **설명과 코멘트**: 티켓 상세에 Jira 설명(ADF)을 렌더링하고, 최근 코멘트 10개를
  보여준다(자동화 봇 코멘트는 숨김, 표는 Jira에서 보라고 안내). 코멘트는 열 때
  가져오고 저장하지 않는다.
- **설명 속 이미지**: 설명에 첨부된 스크린샷·이미지를 상세(윈도우)에서 직접 그린다.
  클릭하면 확대. 티켓만큼 민감한 데이터라 **디스크에 캐시하지 않고** 메모리에만,
  디코딩된 크기 기준 상한으로 보관한다.
- **데스크톱 앱으로 열기**: Slack·Figma 링크는 브라우저 대신 설치된 데스크톱 앱으로
  연다(`slack://`, `figma://`). 앱이 없으면 원래 링크 그대로 브라우저로 간다.
